...
- Each artifact must include:  
  ...
  - Key metadata: e.g. for endpoints, HTTP method and URL; for forms, fields and validation; for SQL, tables and columns. A flexible `details` JSON field should also be included for type-specific attributes.
...
- The system should be resilient to partial failures in extraction or indexing and log errors without aborting the entire run when possible. If an error occurs during a sub-stage, the pipeline should log the error, skip the problematic item, and continue processing remaining items.  
- The pipeline should scale to large monorepos (up to 10 million lines of code or 100,000 files) by processing files in batches and avoiding loading the entire codebase into memory at once. No special handling for secrets found in source code is implemented.

## Clarifications

### Session 2025-12-11

- Q: How should the pipeline handle potential secrets (e.g., API keys, passwords) discovered in the code? → A: Ignore secrets; index all code as-is.
- Q: How should the `all` command behave if an error occurs during one of its sub-stages (discover, extract, index, prd)? → A: Log the error, skip the problematic item, and continue processing.
- Q: What specific fields should be extracted and stored for a generic "code artifact" (e.g., a Java class, a JSP page, an endpoint)? → A: A common set of core metadata plus a flexible `details` JSON field for type-specific attributes.
- Q: What is the estimated maximum size for a "large monorepo" in terms of lines of code or number of files that the pipeline should be designed to handle? → A: Up to 10 million lines of code or 100,000 files.
- Q: How should the pipeline handle connection failures or unavailability of external dependencies (Weaviate, Ollama) during the `index` or `prd` stages? → A: Proceed without the dependency, skipping related operations.

## 7. Assumptions and dependencies

- A Weaviate instance is available and reachable from the machine running the pipeline, either locally or on a remote Linux host. If Weaviate is unavailable during the `index` stage, the pipeline will proceed, skipping the indexing operations.
- Ollama is installed and running on the developer machine and has at least one suitable model pulled for text generation. If Ollama is unavailable during the `prd` stage, the pipeline will proceed, skipping the PRD generation operations.
