#!/usr/bin/env python3
"""
Simplified test script for evaluating all 10 questions using the Flask app endpoints.
This avoids import issues by using HTTP requests to the running Flask app.
"""
...
class QuestionTester:
    def __init__(self, base_url: str = "http://localhost:5000"):
        """Initialize the question tester with Flask app URL."""
        self.base_url = base_url
        self.results = []
        
    def test_question(self, question: str, expected_context_types: List[str] = None) -> Dict[str, Any]:
        """
        Test a single question using the Flask app's chat endpoint.
        ...
        try:
            chat_response = requests.post(f"{self.base_url}/chat", 
                                        json={"message": question})
            ai_response = chat_response.json().get('response', '') if chat_response.status_code == 200 else ''
        except Exception as e:
    ...
    def run_all_tests(self) -> List[Dict[str, Any]]:
        ...
        print(f"Flask App URL: {self.base_url}")
...
def main():
    ...
    print("This script will test all 10 questions against the Flask app")
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Flask app not responding at {base_url}")
            print("Please start the Flask app first using: python flask_app.py")
            return
    except Exception as e:
    ...
    # Initialize tester
    tester = QuestionTester(base_url)
