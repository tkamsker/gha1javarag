#!/bin/bash
# Reset and Run Production Mode - Iteration 14 Complete Pipeline
# This script resets everything and runs the full pipeline in production mode for comprehensive analysis

set -e

echo "🏭 ITERATION 14 - RESET AND RUN PRODUCTION MODE"
echo "==============================================="
echo ""
...
# Start Step 1 with comprehensive logging
(
    echo "=== STEP 1 PRODUCTION LOG - $(date) ===" 
    timeout 1200 ./Step1_Enhanced_Weaviate.sh production 2>&1
) | tee "$LOGDIR/step1_production.log"
...
# Start Step 2 with comprehensive logging
(
    echo "=== STEP 2 PRODUCTION LOG - $(date) ==="
    timeout 900 ./Step2_Enhanced_Weaviate.sh production 2>&1
) | tee "$LOGDIR/step2_production.log"
...
# Start Step 3 with comprehensive logging
(
    echo "=== STEP 3 PRODUCTION LOG - $(date) ==="
    timeout 900 ./Step3_Enhanced_Weaviate.sh production 2>&1
) | tee "$LOGDIR/step3_production.log"
