#!/bin/bash
# Enhanced Step 2 Script - Uses enhanced metadata from Step1_Enhanced
# Exit on error
set -e

# Configuration
MODE=${1:-"test"}  # Default to test mode
echo "🚀 Running Enhanced Step 2 in $MODE mode"
...
if [ -n "$DEBUGFILE" ] && [ -f "$DEBUGFILE" ]; then
    echo "🔍 DEBUG MODE ENABLED for Enhanced Step 2"
    echo "Debug file: $DEBUGFILE"
    echo "Processing enhanced requirements with debug enhancements..."
    
    if [ "$MODE" = "test" ]; then
        echo "Using debug mode with test settings..."
        export RATE_LIMIT_ENV=test
        python3 src/step2_enhanced_debug.py
    ...
    echo "🔧 Running Enhanced Python Step 2: step2_enhanced.py"
    echo "📊 Features enabled:"
    echo "   - Enhanced architectural classification"
    echo "   - Layer-aware requirements analysis"
    echo "   - Component-based documentation"
    echo "   - Domain-driven organization"
    echo "   - Improved rate limiting with $AI_PROVIDER"
    
    if [ "$MODE" = "test" ]; then
        echo "Using test mode for enhanced step2..."
        export RATE_LIMIT_ENV=test
        python3 src/step2_enhanced_test.py
    elif [ "$MODE" = "production" ]; then
        echo "Using production mode for enhanced step2..."
        export RATE_LIMIT_ENV=production
        python3 src/step2_enhanced.py
    elif [ "$MODE" = "emergency" ]; then
        echo "Using emergency mode for enhanced step2..."
        export RATE_LIMIT_ENV=emergency
        python3 src/step2_enhanced.py
    else
        echo "Using test mode for enhanced step2..."
...
if [ $? -ne 0 ]; then
    if [ "$AI_PROVIDER" = "openai" ]; then
        ...
        echo ""
        echo "💡 Tips:"
        echo "   - Try running in 'emergency' mode: ./Step2_Enhanced.sh emergency"
...
echo "✅ Enhanced Step 2 completed successfully in ${STEP2_TIME} seconds"
echo "Mode used: $MODE"
echo "AI Provider: $AI_PROVIDER"
echo "Enhanced features: Architectural classification, layer-aware analysis"
...
echo "   - Layer-Aware Analysis: Enabled"
echo "   - Component-Based Documentation: Enabled"
...
echo "   - Layer summaries: ./output/layer_summary_*.json"
