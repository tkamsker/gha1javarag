...
class TestDebugRequirementsProcessor:
    def __init__(self):
        ...
        self.requirements_file = os.path.join(self.output_dir, "modern_requirements_debug_test.md")
    ...
    async def modernize_requirements(self, content: dict, file_path: str = "") -> dict:
        """Use AI to modernize requirements for cloud architecture with debug and test mode optimizations"""
        # Simplified prompt for debug test mode
        prompt = f"""
        ...
        5. Basic data models
        6. Debug and monitoring considerations (if debug mode)
        ...
        4. API Endpoints
        5. Data Models
        6. Debug Considerations (if debug mode)
        """
        
        try:
            analysis = await self.ai_provider.create_chat_completion(
                messages=[
                    {"role": "system", "content": "You are an expert in modern cloud architecture and debugging. Provide simplified analysis for test mode with debug considerations."},
            ...
            try:
                ...
                return {
                    "Modern Architecture Overview": "Generated from AI analysis (debug test mode)",
                    "Frontend Components (React)": "Generated from AI analysis (debug test mode)", 
                    "Backend Services (Node.js)": "Generated from AI analysis (debug test mode)",
                    "API Endpoints": "Generated from AI analysis (debug test mode)",
                    "Data Models": "Generated from AI analysis (debug test mode)",
                    "Debug Considerations": "Generated from AI analysis (debug test mode)" if self.debug_mode else "Not applicable",
            ...
            return {
                "Modern Architecture Overview": "Error occurred during modernization (debug test mode)",
                "Frontend Components (React)": "Error occurred during modernization (debug test mode)",
                "Backend Services (Node.js)": "Error occurred during modernization (debug test mode)", 
                "API Endpoints": "Error occurred during modernization (debug test mode)",
                "Data Models": "Error occurred during modernization (debug test mode)",
                "Debug Considerations": "Error occurred during modernization (debug test mode)" if self.debug_mode else "Not applicable",
                "Original Content": content
            }
    
    async def generate_requirements_document(self) -> None:
        """Generate the final requirements document with debug and test mode information"""
        with open(self.requirements_file, 'w', encoding='utf-8') as f:
            f.write("# Modern Cloud Architecture Requirements (Debug Test Mode)\n\n")
            ...
            if self.debug_mode:
                f.write("## Debug Insights (Test Mode)\n\n")
                f.write("### Processing Summary\n\n")
                f.write(f"- **Debug Mode:** Enabled\n")
                f.write(f"- **Debug File:** {self.debug_file}\n")
                f.write(f"- **Files with Enhanced Metadata:** {len([f for f in self.processed_content.keys() if 'pom_analysis' in f])}\n\n")
                
                f.write("### Recommendations for Debug Mode (Test)\n\n")
                f.write("1. **Basic Monitoring:** Implement simple logging and monitoring\n")
                f.write("2. **Debug Tools:** Use basic debugging tools\n")
                f.write("3. **Error Handling:** Implement basic error handling\n")
                f.write("4. **Testing:** Focus on basic unit testing\n")
                f.write("5. **Documentation:** Maintain basic documentation for debugging\n\n")
            ...
            if self.debug_mode:
                f.write("- **Debug Enhancements:** Includes debug-specific analysis and recommendations\n\n")
    
    async def process_all_files(self) -> None:
        """Process all markdown files and generate requirements with debug and test mode limits"""
        # Process web files first (limit to first 2 for test mode)
...
async def main():
    processor = TestDebugRequirementsProcessor()
    ...
    print(f"Debug test requirements document generated at: {processor.requirements_file}")
