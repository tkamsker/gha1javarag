...
- Updates the product_index values for product offerings in the cct_productoffering table for version 20.12, with some modifications from the previous version
- Updates the product_index values for specific product offerings in the cct_productoffering table to establish a defined display/processing order
...
#### product_index_update
Description: Series of UPDATE statements that assign sequential index values to product offerings with some different mappings from previous version
...
- Previous version (58_CuCo_V20.11) should be considered for change tracking

## 5. Business Rules
- Index partitions must be disabled by month
- All indexes listed in custc_indexes must be dropped
- Indexes must be created in order by table name
- Account phone numbers must be unique per customer and account
- Product offerings must have sequential index values starting from 1
- Specific product IDs are mapped to specific index values for ordering
- Product offerings must have sequential index values starting from 1
- Updated mapping of product IDs to index values, with some differences from previous version
